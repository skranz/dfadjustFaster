# This version contiains two modifications:
# 1. change the apply / tapply calls using collapse::fsum
# 2. Add for_coefs argument
# 3. Add inv_mode argument. Can distinguish between inverting
#    for each cluster s a K x K matrix or a n_s x n_s matrix
...
#'     or for the coefficients in \code{for_coefs} if not NULL,
#' @param for_coefs Either NULL or an integer vector specifying the
#'  coefficients of interest.
#'  Restricting the set of coefficients will typically speed up the
#'   computation. The argument is only considered if \code{ell} is NULL.
...
#'     \code{FALSE}).
#'  @param inv_mode Only used for cluster robust se. If "K" then for each cluster invert a K x K matrix for each cluster where K is the number of columns in X.  If "n_s" then for each cluster s invert a n_s x n_s matrix, where n_s is the number of rows in cluster s. If "min" check for each cluster  whether K or n_s is smaller and invert the smaller matrix. The code for inv_mode = "K" corresponds to the original procedure implemented in Kolesar (2023). The modes "n_s" and "min" are a small adaption by Sebastian Kranz based on the exposition in the vignette  Kolesar (2023). For backward compatibility the default is "K", but using "min" is recommended for speed.
...
#' @export
dfadjustSE <- function(model, clustervar=NULL, ell=NULL,for_coefs=NULL, IK=TRUE, inv_mode = c("K","n_s", "min")[1], tol=1e-9,rho0=FALSE, timing=FALSE) {
    start_time = Sys.time()
    ...
    if (is.null(clustervar)) {
        ...
        .inv_mode = inv_mode
        AQf <- function(s) {
          rows = which(clustervar==s)
          n_s = length(rows)
          Qs <- Q[rows, , drop=FALSE] # nolint
          if (inv_mode == "min") {
            # Depending on cluster size n_s
            # invert the smaller of a
            # K x K or n_s x n_s matrix
            .inv_mode = if (K <= n_s) "K" else "n_s"
          }
          if (.inv_mode == "K") {
            # Original method by Kolesar (2023)
            # eigen decomposition of  K x K matrix
            e <- eigen(crossprod(Qs),symmetric = TRUE)
            Ds <-  (1-e$values >= tol) *
              (1/sqrt(pmax(1-e$values, tol))) * t(e$vectors)
            AQ =  Qs %*% e$vectors %*% Ds
          } else if (.inv_mode == "n_s") {
            # Modified version of Kolesar (2023)
            # eigen decomposition of n_clu[s] x n_clus[s] matrix
        ...
        df0 <- function(ell) {
            ...
            if (!IK) {
                ...
                # Fm can be computed outside the loop
    ...
    } else if (!is.null(for_coefs)) {
        se <- sqrt(diag(Vhat))[for_coefs]
        dof <- vapply(for_coefs, function(k) df0(diag(K)[, k]), numeric(1))
        seStata <- sqrt(diag(VhatStata))[for_coefs]
        beta <-  model$coefficients[for_coefs]
    } else {
        se <- sqrt(diag(Vhat))
